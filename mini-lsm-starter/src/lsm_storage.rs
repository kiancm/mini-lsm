...
impl LsmStorageInner {
    ...
    pub fn scan(
        ...
        let state = self.state.read();
        let memtable_iters = std::iter::once(state.memtable.scan(lower, upper))
            .chain(
                state
                    .imm_memtables
                    .iter()
                    .map(|mt| mt.as_ref().scan(lower, upper)),
            )
            .map(Box::new)
            .collect();
        LsmIterator::new(MergeIterator::create(memtable_iters)).map(FusedIterator::new)
    }
}
